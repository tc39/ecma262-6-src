...
def version_is_6(docx):
    return os.path.basename(docx.filename).lower().startswith('es6')

def version_is_6_final(docx):
    return os.path.basename(docx.filename) == 'es6-final.docx'
...
tag_names = {
    ...
    'BulletNotlast': 'li',
    'Caption': 'figcaption',
    'CodeSample3': '.NoteCode',
    'copyrightlast': 'p.copyrightlast',
    'DateTitle': 'h1',
    'ECMAWorkgroup': 'h1.ECMAWorkgroup',
    'Ecmaaddress': 'p.ECMAaddress',
    'Example': '.Note',
    ...
    'MathSpecialCase3': 'li',
    'normalbefore': 'p.normalbefore',
    'NormalBullet': 'p.normalBullet',
    'NormalBulletSubstep': 'p.normalBulletSubstep',
    'Note': '.Note',
    'NoteAfter': '.Note',
    'NoteCode': '.NoteCode',
    'Note1st': '.Note1st',
...
@Fixup
def fixup_remove_TOC(doc, docx):
    def remove_if_TOC(p):
        cls = p.attrs.get('class')
        if cls != None and cls[:3] == 'TOC':
            return []
        else:
            return [p]

    return doc.replace('p', remove_if_TOC)
        ...
        for k in parent.content:
            if isinstance(k, str):
                ...
                with marker_temporarily_removed(k):
                    ...
                    if k.content:
                        a = k.content[0]
                        if isinstance(a, str) and a[:1].isspace() and a[:1] != '\u00a0':
...
@InPlaceFixup
def fixup_insert_special_sections(doc, docx):
    """ Wrap the copyright notices with a div and bibliography with sections. """
    
    def match_Ecmaaddress(e):
        if e.name == 'p' and e.attrs.get('class') == 'ECMAaddress':
            return True  
        return False
        
    def match_copyright_last(e):
        if e.name == 'p' and e.attrs.get('class') == 'copyrightlast':
            return True  
        return False
    ...
    while j < len(body):
        kid = body[j]
        if first == None and match_Ecmaaddress(kid):
            first = j
            break
        j += 1
    if first != None:
    ...
    if last is not None:
        body_elt.content[first:last] = [html.div(*body_elt.content[first:last], class_="copyright")]    
    else:
        warn("last not found");
...
def fixup_strip_toc(doc, docx):
    """ Delete the table of contents in the document, if it is still there.
    ...
    if version_is_6_final(docx):
        #skip forward until a <hr> that is preceded by a "Contents" paragraph
        while not ht_text(body.content[i0 - 1]).startswith('Contents'):
            i0, next_hr = next(hr_iterator)
        i1=i0
        i0=i0 - 1
...
def fixup_notes(doc, docx):
    ...
    for parent, i, p in all_parent_index_child_triples(doc):
        if p.name == 'p':
            ...
            has_note1st_class = p.attrs.get('class') == 'Note1st'
            nh_info = find_nh(p, strict=has_note_class or has_note1st_class)
            if nh_info:
                ...
                if has_note_class or has_note1st_class:
                    del p.attrs['class']

                # Look for sibling elements that belong to the same note.
                j = i + 1
                if has_note1st_class:
                    seen_last = False
                    while j < len(parent.content) and not seen_last:
                        this_class = parent.content[j].attrs.get('class')
                        if this_class in ('Note', 'Note1st'):
                            seen_last = True
                        else:
                            seen_last = this_class == 'NoteLast'
                            if this_class in ('NoteLast', 'Note1st', 'NoteMore'):
...
@InPlaceFixup
def fixup_table_Import_Forms_Mappings(doc, docx):
    """ Convert last row of table titled 'Import Forms Mappings to ImportEntry Records'
    so it's last <td> has a colspan='3' attribute """

    table_count = 0
    
    def match_section(e):
        if is_section_with_title(e, "Source Text Module Records"):
            return True
        elif e.name in ('section', 'body', 'html'):
            return False  # no match, but visit children
        else:
    ...
    def match_3rd_table(e):
        ...
        if e.name == 'table':
            ...
            if table_count == 3:
                ...
                return None
        elif e.name in ('section',  'figure'):
            return False  # no match, but visit children
        else:
            return None  # no match and skip entire subtree
            
    sec = next(doc.find(match_section))
    tab = next(sec.find(match_3rd_table))
    if tab == None :
        Warn("Table not found: Source Text Module Records")
    else:
        tr6 = tab.content[5]
        td2 = tr6.content[1]
        td2.attrs['colspan'] = '3'
...
def fixup_remove_margin_style(doc, docx):
    ...
    def has_margins(e):
        classAttr = e.attrs.get('class')
        special = classAttr is not None and classAttr in ('special1')
        return not special and e.style is not None and any(is_margin_property(k) for k in e.style)
...
def fixup_links(doc, docx):
    ...
    specific_link_source_data_lang = [
        ...
        # 12.3.4.1
        #("direct eval", "Function Calls Runtime Semantics Evaluation"),
        
        # 15.2
		("Source Text Module Record", "Source Text Module Records"),
		("Source Text Module Records", "Source Text Module Records"),
...
def fixup_add_disclaimer(doc, docx):
    ...
    if spec_is_lang(docx) and version_is_51_final(docx):
        disclaimer = div(
            ...
            id="unofficial")
        position = 1
    elif spec_is_lang(docx) and version_is_6_final(docx):
        disclaimer = div(
            p("This is the HTML rendering of ", i("ECMA-262 6", sup("th"), " Edition, The ECMAScript 2015 Language Specification"), "."),
            p("The PDF rendering of this document is located at ",
              a("http://www.ecma-international.org/ecma-262/6.0/ECMA-262.pdf",
                href="http://www.ecma-international.org/ecma-262/6.0/ECMA-262.pdf"),
              "."),
            p("The PDF version is the definitive specification. Any discrepancies between this HTML version "
              "and the PDF version are unintentional."),
            id="unofficial")
...
def fixup_add_ecma_flavor(doc, docx):
    if version_is_intl_1_final(docx) or version_is_6_final(docx):
        ...
        hgroup.content[2].style["font-size"] = "225%"
...
def get_fixups(docx):
    ...
    yield fixup_sec_4_3
    yield fixup_hr
    if spec_is_intl(docx) or version_is_6_final(docx):
        yield fixup_intl_remove_junk
    if version_is_6_final(docx):
        yield fixup_insert_special_sections
    ...
    if spec_is_lang(docx):
        yield fixup_table_Import_Forms_Mappings
